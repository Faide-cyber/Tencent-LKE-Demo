...
bce-auth-v1/giKjaxxxxxxxxxGS9rs/2024-10-11T06:36:08Z/180000000700000/host;x-bce-date/a4181xxxxxxxxxcb5f09629f2546b7ca04";//API密钥，APIPassword
	...
	}else if($postData_arr["model"] == "deepseek-v3") {
    $my_CURLOPT_URL = "https://wss.lke.cloud.tencent.com/v1/qbot/chat/sse";
    ...
    // 修正POST字段 (关键修改)
    $postData = json_encode($postData_arr); // 直接传递已构建的腾讯云格式数据
...
$callback = function ($ch, $data) use (&$isDeepSeek) {
    if ($isDeepSeek) {
        $event = '';
        $payload = '';
        ...
        // 处理reply事件
        if ($event === 'reply' && isset($payload['payload']['content'])) {
            // 转换为OpenAI兼容格式
            $openaiFormat = [
                'choices' => [[
                    'delta' => ['content' => $payload['payload']['content']]
                ]]
            ];
...
// 在cURL配置前添加DeepSeek标识
$isDeepSeek = ($postData_arr["model"] == "deepseek-v3");
...
if ($statusCode >= 400) {
    setcookie("errcode", "http_error_".$statusCode);
    setcookie("errmsg", "HTTP错误：".$statusCode);
}
...
file_put_contents('curl.log', $verboseLog, FILE_APPEND);
$answer = "";
